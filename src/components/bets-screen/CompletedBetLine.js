...
import CompletedMatchLine from './CompletedMatchLine';

const Wrapper = styled.SafeAreaView`
  background-color: ${(props) =>
    props.success ? props.theme.colors.secondary : '#920101'};
  border-radius: 8px;
  margin-bottom: 5px;
  margin-top: 5px;
`;
...
const ActiveBetLine = ({bet}) => {
  ...
  return (
    <Wrapper success={bet.state === 'success'}>
      <TouchableWithoutFeedback onPress={() => setDetail(!detail)}>
        <FirstLine>
          <Title>{bet.state === 'success' ? 'Kazandı' : 'Kaybetti'}</Title>
      ...
      {detail ? (
        <View style={{borderBottomWidth: 3, borderColor: '#DEE1EC'}}>
          <FlatList
            ...
            renderItem={({item}) => <CompletedMatchLine match={item} />}
          />
        </View>
      ) : (
        <View />
